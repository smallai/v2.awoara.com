...
@section('content')
    ...
    @include('wechat.deviceInfo')

        <div class="weui-msg__text-area">
            <h2 class="weui-msg__title"></h2>
            <div class="weui-panel">
                <form action="{{ route('wechat.order') }}" method="POST">
                    {{ csrf_field() }}
                    {{ method_field('POST') }}
                    <input type="hidden" name="device_id" id="device_id" value="{{ $device->id }}">
                    <div class="weui-cells__title">套餐选项</div>
            ...
            <ul>
                <div class="weui-cells">
                    ...
                    @if (isset($card))
                        ...
                        @if (UserHaveNotMatchVipCard($card, $device))
                            <div class="weui-cell weui-cell_warn">
                                <ol>您的会员卡不适用于该设备,请付费使用！(会员卡不通用！)</ol>
                            </div>
                        @elseif (UserHaveExpiredVipCard($card, $device))
                            <div class="weui-cell weui-cell_warn">
