...
//        }
    ...
    protected function createWithdrawMoneyRecord($owner, $device, $trades)
    {
        ...
        $withdraw->log_user_login_id = isset($logOwnerLastLogin) ? $logOwnerLastLogin->id : null;

        $withdraw->withdraw_status = Trade::WithdrawStatus_Processing;
        $withdraw->withdraw_at = Carbon::now();
        ...
        $withdraw->payer_remark = $withdraw->payer_show_name.':'.$device->name.',收益:'.to_float($this->param['withdraw_money']).'元';
        $withdraw->payment_type = Trade::PaymentType_Alipay;
        $withdraw->updateInfo();
//        $withdraw->save();
        Log::debug('end create record');
        debugbar()->debug($withdraw);
        return $withdraw;
    }
...
//"pay_date" => "2018-05-25 12:55:54"
    public function payment($withdraw, $trades)
    {
        ...
        try
        {
            ...
            $withdraw->payment_at = $result['pay_date'];
...
//        try
        {
            ...
            $result = Trade::whereIn('id', $trades->pluck('id'))->update([
                ...
                'withdraw_at' => $withdraw->withdraw_at,
            ]);
            \DB::commit();
        }
//        catch (\Exception $exception)
